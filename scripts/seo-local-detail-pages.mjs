...
function renderStaticRouteFeedItem([title, href, description]) {
...
function writeFeed() {
  ...
  const generatedLinks = new Set([
    ...pages.map(pageUrl),
    ...staticRouteFeedItems.map(([, href]) => `${siteUrl}${href}`)
  ]);
  ...
  const items = [
    ...
    ...staticRouteFeedItems.map(renderStaticRouteFeedItem),
